...
public func trustedDealerKeygenFrom(configuration: Configuration)throws  -> TrustedKeyGeneration {
    return try  FfiConverterTypeTrustedKeyGeneration.lift(try rustCallWithError(FfiConverterTypeConfigurationError.lift) {
...
private var initializationResult: InitializationResult {
    ...
    if (uniffi_frost_uniffi_sdk_checksum_func_aggregate() != 50754) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_frost_uniffi_sdk_checksum_func_generate_nonces_and_commitments() != 11863) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_frost_uniffi_sdk_checksum_func_new_signing_package() != 59539) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_frost_uniffi_sdk_checksum_func_sign() != 42997) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_frost_uniffi_sdk_checksum_func_trusted_dealer_keygen_from() != 50670) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_frost_uniffi_sdk_checksum_func_trusted_dealer_keygen_with_identifiers() != 48885) {
        return InitializationResult.apiChecksumMismatch
    }
