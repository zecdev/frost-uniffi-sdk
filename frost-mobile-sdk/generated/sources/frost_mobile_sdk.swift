...
#if canImport(frost_mobile_sdkFFI)
import frost_mobile_sdkFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_frost_mobile_sdk_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_frost_mobile_sdk_rustbuffer_free(self, $0) }
    }
}
...
public struct FrostPublicKeyPackage {
    public var verifyingShares: [String: String]
    public var verifyingKey: String

    // Default memberwise initializers are never public by default, so we
    // declare one manually.
    public init(verifyingShares: [String: String], verifyingKey: String) {
    ...
    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> FrostPublicKeyPackage {
        return
            try FrostPublicKeyPackage(
                verifyingShares: FfiConverterDictionaryStringString.read(from: &buf), 
                verifyingKey: FfiConverterString.read(from: &buf)
        )
    }

    public static func write(_ value: FrostPublicKeyPackage, into buf: inout [UInt8]) {
        FfiConverterDictionaryStringString.write(value.verifyingShares, into: &buf)
...
public func validateConfig(config: Configuration)throws  {try rustCallWithError(FfiConverterTypeConfigurationError.lift) {
    uniffi_frost_mobile_sdk_fn_func_validate_config(
...
public func verifyAndGetKeyPackageFrom(secretShare: FrostSecretKeyShare)throws  -> FrostKeyPackage {
    ...
    uniffi_frost_mobile_sdk_fn_func_verify_and_get_key_package_from(
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_frost_mobile_sdk_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_frost_mobile_sdk_checksum_func_validate_config() != 41769) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_frost_mobile_sdk_checksum_func_verify_and_get_key_package_from() != 49233) {
        return InitializationResult.apiChecksumMismatch
    }
